...
-- Parser function to convert custom BBCode-like tags to Roblox RichText
local function ParseBBToRich(text)
    local result = ""
    local i = 1
    local len = #text
    local current_color = false
    local stack = {}  -- To track open tags for proper closing
    ...
    while i <= len do
        ...
        if char == "[" then
            ...
            if j then
                local tag = text:sub(i + 1, j - 1):upper()  -- Uppercase for hex consistency
                i = j + 1
                if tag:match("^%x%x%x%x%x%x$") then  -- 6 hex digits for color
                    local r = tonumber(tag:sub(1, 2), 16)
                    local g = tonumber(tag:sub(3, 4), 16)
                    local b = tonumber(tag:sub(5, 6), 16)
                    if current_color then
                        result = result .. "</font>"
                    end
                    result = result .. '<font color="rgb(' .. r .. ',' .. g .. ',' .. b .. ')">'
                    current_color = true
                elseif tag:match("^%x%x$") then  -- 2 hex digits for grayscale
                    local gray = tag
                    local v = tonumber(gray, 16)
                    if current_color then
                        result = result .. "</font>"
                    end
                    result = result .. '<font color="rgb(' .. v .. ',' .. v .. ',' .. v .. ')">'
                    current_color = true
                elseif tag == "RGB" then  -- Reset color
                    if current_color then
                        result = result .. "</font>"
                        current_color = false
                    end
                elseif tag:match("^[%w]+/$") then  -- Closing tag like [b/]
                    local close_tag = tag:sub(1, -2):lower()
                    result = result .. "</" .. close_tag .. ">"
                elseif tag:match("^[%w]+$") then  -- Opening tag like [b], [i], etc.
                    local open_tag = tag:lower()
                    if open_tag == "c" then
                        result = result .. '<font face="Code">'
                    else
                        result = result .. "<" .. open_tag .. ">"
                    end
                    table.insert(stack, open_tag)
                else
                    result = result .. "[" .. tag .. "]"
                end
            else
